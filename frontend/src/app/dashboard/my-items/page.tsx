        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-sky-500">
              Order Penitipan Saya
            </h1>
            <p className="text-slate-400 mt-1">
              Lihat dan kelola semua order penitipan yang telah Anda buat.
            </p>
          </div>
          <Link
        ...
        {!isLoadingOrders && !error && orders.length === 0 && (
          <div className="text-center py-10 bg-slate-800/50 rounded-lg">
            ...
            <p className="text-xl text-slate-400">
              Anda belum membuat order penitipan.
            </p>
            <p className="text-slate-500 mt-1">
              Mulai titipkan barang berharga Anda sekarang.
            </p>
          </div>
        )}
        ...
        {!isLoadingOrders && !error && orders.length > 0 && (
          <div className="space-y-6">
            {orders.map((order) => (
              <div
                key={order.id}
                className="bg-slate-800 rounded-xl shadow-lg overflow-hidden hover:shadow-sky-500/20 transition-shadow duration-300"
              >
                  ...
                  <div className="flex-1 p-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start mb-4">
                      <div>
                        ...
                        <span
                          className={`inline-flex items-center px-3 py-1 text-sm font-semibold rounded-full border ${getStatusColor(
                            order.status
                          )}`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div className="flex items-center text-sm text-slate-300">
                        ...
                        <span>
                          {getTotalItems(order)} barang (
                          {order.entrustedItems.length} jenis)
                        </span>
                      </div>
                      ...
                      <div className="flex items-center text-sm text-slate-300">
                        ...
                        <span>
                          {getMonitoringFrequencyText(
                            order.monitoringFrequency
                          )}
                        </span>
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        {order.entrustedItems.length > 3 && (
                          <p className="text-xs text-slate-500">
                            ... dan {order.entrustedItems.length - 3} barang
                            lainnya
                          </p>
                        )}
                      </div>
                    ...
                    <div className="mb-4">
                      <div className="flex items-start text-sm text-slate-300">
                        <MapPin
                          size={16}
                          className="mr-2 mt-0.5 text-emerald-400 flex-shrink-0"
                        />
