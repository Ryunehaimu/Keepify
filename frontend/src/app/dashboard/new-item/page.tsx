...
import { ArrowLeft, PackagePlus, UploadCloud, Plus, Trash2, Clock, MapPin, Phone } from 'lucide-react';
...
const entrustedItemSchema = z.object({
  ...
  description: z.string().min(10, 'Deskripsi minimal 10 karakter').max(500, 'Deskripsi maksimal 500 karakter').optional(),
  category: z.string().min(1, 'Kategori harus diisi').max(100, 'Kategori maksimal 100 karakter').optional(),
  estimatedValue: z.unknown()
    .transform((val, ctx) => {
    ...
    .optional(),
  itemCondition: z.string().min(3, 'Kondisi barang minimal 3 karakter').optional(),
...
const entrustmentOrderSchema = z.object({
  ...
  pickupAddress: z.string().min(10, 'Alamat penjemputan minimal 10 karakter').max(500, 'Alamat maksimal 500 karakter'),
  contactPhone: z.string().min(10, 'Nomor telepon minimal 10 digit').max(20, 'Nomor telepon maksimal 20 karakter'),
  expectedRetrievalDate: z.string().optional(),
  entrustedItems: z.array(entrustedItemSchema).min(1, 'Harus ada minimal satu barang yang dititipkan'),
}).refine((data) => {
...
export default function NewEntrustmentOrderPage() {
  ...
  const onSubmit: SubmitHandler<EntrustmentOrderFormData> = async (data) => {
    ...
    try {
      ...
      setImagePreview(null);
      setPackageImage(null);
      const fileInput = document.getElementById('packageImageInput') as HTMLInputElement;
  ...
  if (!isAuthenticated) {
    return <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white">Mengalihkan ke halaman login...</div>;
  }
    ...
    <div className="min-h-screen bg-slate-900 text-white p-4 sm:p-8">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="bg-slate-800 p-6 sm:p-8 rounded-lg shadow-2xl">
          ...
          {serverError && (
            <div className="bg-red-500/20 border border-red-600 text-red-200 px-4 py-3 rounded-lg mb-6" role="alert">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="bg-slate-700/30 p-6 rounded-lg">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-xl font-semibold text-emerald-400">Informasi Penjemputan</h2>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="pickupRequestedDate" className="block text-sm font-medium text-slate-300 mb-1.5">
                    Tanggal & Jam Penjemputan <span className="text-red-400">*</span>
                  </label>
                  <input
              ...
              <div className="mt-6">
                <label htmlFor="pickupAddress" className="block text-sm font-medium text-slate-300 mb-1.5">
              ...
              <div className="mt-6">
                <label htmlFor="expectedRetrievalDate" className="block text-sm font-medium text-slate-300 mb-1.5">
                  Perkiraan Tanggal Pengambilan <span className="text-xs text-slate-400">(Opsional)</span>
                </label>
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                />
                {errors.expectedRetrievalDate && <p className="text-red-400 text-xs mt-1.5">{errors.expectedRetrievalDate.message}</p>}
              </div>
            </div>
            ...
            <div className="bg-slate-700/30 p-6 rounded-lg">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-xl font-semibold text-purple-400">Preferensi Monitoring</h2>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="allowChecks" className="ml-3 text-sm text-slate-300">
                ...
                {allowChecks && (
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">
                      Frekuensi Monitoring <span className="text-red-400">*</span>
                    </label>
                    <div className="space-y-2">
                      {Object.values(MonitoringFrequency).map((freq) => (
                        <div key={freq} className="flex items-center">
                          ...
                          />
                          <label htmlFor={`monitoring-${freq}`} className="ml-3 text-sm text-slate-300">
                    ...
                    {errors.monitoringFrequency && <p className="text-red-400 text-xs mt-1.5">{errors.monitoringFrequency.message}</p>}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-700/30 p-6 rounded-lg">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <h2 className="text-xl font-semibold text-sky-400">Daftar Barang yang Dititipkan</h2>
                </div>
                <button
                  type="button"
                  onClick={addItem}
              ...
              <div className="space-y-6">
                {fields.map((field, index) => (
                  <div key={field.id} className="bg-slate-600/30 p-4 rounded-lg relative">
                    {fields.length > 1 && (
                      <button
                        type="button"
                    ...
                    <h3 className="text-lg font-medium text-slate-200 mb-4">Barang {index + 1}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        {errors.entrustedItems?.[index]?.name && (
                          <p className="text-red-400 text-xs mt-1">{errors.entrustedItems[index]?.name?.message}</p>
                        )}
                      </div>
              ...
              {errors.entrustedItems?.root && (
                <p className="text-red-400 text-xs mt-2">{errors.entrustedItems.root.message}</p>
              )}
            </div>

            {/* Package Image Section */}
            <div>
              <label htmlFor="packageImageInput" className="block text-sm font-medium text-slate-300 mb-1.5">
                Foto Paket/Box Keseluruhan <span className="text-xs text-slate-400">(Opsional, maks. 5MB)</span>
              </label>
              <div className="mt-1 flex flex-col items-center justify-center w-full px-6 pt-5 pb-6 border-2 border-slate-600 border-dashed rounded-md hover:border-sky-500 transition-colors">
                {imagePreview ? (
                  <img src={imagePreview} alt="Pratinjau Gambar Paket" className="mx-auto h-36 w-auto rounded-md object-contain mb-3"/>
                ) : (
                ...
                <div className="space-y-1 text-center">
                  <div className="flex text-sm text-slate-400 justify-center">
                    <label
                      ...
                      className="relative cursor-pointer rounded-md font-medium text-sky-400 hover:text-sky-300 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-slate-800 focus-within:ring-sky-500 px-1"
                    >
                    ...
                    <p className="pl-1 hidden sm:inline">atau tarik dan lepas</p>
                  </div>
                  <p className="text-xs text-slate-500">PNG, JPG, WEBP hingga 5MB</p>
                </div>
              </div>
              {packageImage && <p className="text-slate-400 text-xs mt-1.5">File terpilih: {packageImage.name} ({(packageImage.size / 1024).toFixed(1)} KB)</p>}
            </div>
