  ...
  const handleSaveUser = async (updatedUser: UserType) => {
    try {
      await apiClient.post(
        `/admin/users/${updatedUser.id}/update`,
        updatedUser
      );
      setUsers((prev) =>
          ...
          <table className="min-w-full divide-y divide-slate-700">
            <thead className="bg-slate-900">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm font-medium text-slate-300 uppercase">
                  Email
                </th>
                <th
                  className="px-6 py-3 text-left text-sm font-medium text-slate-300 uppercase cursor-pointer"
      ...
      {isModalOpen && editingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-slate-800 p-6 rounded-lg w-96 relative">
            ...
            <input
              type="text"
              className="w-full px-3 py-2 rounded-lg bg-slate-700 mb-4 text-white"
            ...
            <div className="relative mb-4">
              <select
                className="w-full px-3 py-2 pr-10 rounded-lg border-2 bg-slate-800 border-slate-700 text-white appearance-none focus:outline-none focus:border-sky-400"
